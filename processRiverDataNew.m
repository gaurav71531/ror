function processRiverDataNew(varargin)
...
%             [distOfOccupied(occRorInd),clusterTotDemPdfTemp(occRorInd)] = getTotalDistance(demClusterInd, rivPt(rorOccupiedSet(occRorInd)), 'ind', []);
            [distOfOccupied(occRorInd),clusterTotDemPdfTemp(occRorInd),clusterVars] = ...
                getTotalDistanceNew(D, occRorInd, 'add', dInd, rivPt(rorOccupiedSet(occRorInd)), clusterVars);
...
%     first assignment
    ...
    %         assign to new ror site
            ...
            clusterVars = updateClusterVars(clusterVars, rorAllocated, D{rorAllocated}, 'add', dInd, rivPt(rorAvailableSet(indMinAvail)));
    ...
    % % %         LOG
            ...
            clusterVars{rorAllocated,3} = [clusterTotDemPdf(rorAllocated).nzStartInd,...
                ...
                clusterTotDemPdf(rorAllocated).std];
        else
    %         assign to existing ones
            clusterVars = updateClusterVars(clusterVars, indMinOcc, D{indMinOcc}, 'add', dInd, rivPt(rorOccupiedSet(indMinOcc)));
            ...
            clusterVars{indMinOcc,3} = [clusterTotDemPdf(indMinOcc).nzStartInd, ...
                ...
                clusterTotDemPdf(indMinOcc).mean, ...
                clusterTotDemPdf(indMinOcc).std];
            % % % LOG
...
%             [distClusterRef(occRorInd), clusterTotDemPdfRef(occRorInd)] = getTotalDistance(demClusterInd, rivPt(rorOccupiedSet(occRorInd)), 'ind', []);
            [distClusterRef(occRorInd), clusterTotDemPdfRef(occRorInd),clusterVars] = ...
                getTotalDistanceNew(DTempRef, occRorInd, 'rem', dInOccRorInd, rivPt(rorOccupiedSet(occRorInd)), clusterVars);
        else
...
%                 try
                ...
                [distClusterTemp(occRorIndNext), clusterTotDemPdfTemp(occRorIndNext),clusterVars] = ...
                getTotalDistanceNew(DTemp, occRorIndNext, 'add', dInOccRorInd, rivPt(rorOccupiedSet(occRorInd)), clusterVars);
%                 catch
%                 end
            else
...
%             
            ...
            clusterVars{reshuffleNextInd,3} = [clusterTotDemPdf(reshuffleNextInd).nzStartInd, ...
                ...
                clusterTotDemPdf(reshuffleNextInd).mean, ...
                clusterTotDemPdf(reshuffleNextInd).std];
%             
            distTotal = sum(distCluster);
            DTemp = D;
%             
            clusterVars = updateClusterVars(clusterVars, occRorInd, DTemp{occRorInd}, 'rem', dInOccRorInd, rivPt(rorOccupiedSet(occRorInd)));
%             
            DTemp{occRorInd}(DTemp{occRorInd} == dInOccRorInd) = [];
%             
            clusterVars = updateClusterVars(clusterVars, reshuffleNextInd, DTemp{reshuffleNextInd}, 'add', dInOccRorInd, rivPt(rorOccupiedSet(reshuffleNextInd)));
%             
...
% [distOfOccupied(occRorInd),clusterTotDemPdfTemp(occRorInd)] = getTotalDistanceNew('base', D{occRorInd}, 'add', dInd, rivPt(rorOccupiedSet(occRorInd)))

function [val, demPdf, clusterVars] = getTotalDistanceNew(D, demClusterBaseInd, task, dInd, rivPt, clusterVars)
...
if strcmp(task, 'add')
    clusterVars{demClusterBaseInd,1} = checkClusterFFTLength(clusterVars{demClusterBaseInd,1}, D{demClusterBaseInd}, lenSinglePdf);
    ...
    euclideanDist = clusterVars{demClusterBaseInd,2} + (demPt(dInd).xPos - rivPt.xPos)^2 ...
...
y = ifft(fftUse, lenFFT);

y = y(1:lenPdf) * delXInPdf;

[s, e] = getNZInterval(y);
demPdf.nzStartInd = numZerosBegin + s; 
demPdf.y = y(s:e);
demPdf.x = [];
demPdf.fft = [];
demPdf.nzEndInd = demPdf.nzStartInd + length(demPdf.y) - 1;
demPdf.mean = mean;
demPdf.std = std;
...
function fftOut = checkClusterFFTLength(fftIn, clusterInd, len)

global demPt;

lenConv = (length(clusterInd) + 1)*len - length(clusterInd);
if length(fftIn) >= lenConv
    fftOut = fftIn;
else
    lenFFT = pow2(nextpow2(lenConv));
    fftInd = log2(lenFFT) - 5;
    fftOut = demPt(clusterInd(1)).pdf.fft(fftInd,1:lenFFT);
    for i = 2:length(clusterInd)
        fftOut = fftOut .* demPt(clusterInd(i)).pdf.fft(fftInd,1:lenFFT);
    end
end


function clusterVars = updateClusterVars(clusterVars, baseInd, clusterInd, task, dInd, rivPt)


global demPt;

len = length(demPt(1).pdf.y);

if strcmp(task, 'add')
    lenConv = (length(clusterInd) + 1)*len - length(clusterInd);
    lenFFT = max(pow2(nextpow2(lenConv)), length(clusterVars{baseInd,1}));
    fftInd = log2(lenFFT) - 5;
    if length(clusterVars{baseInd,1}) >= lenConv
        clusterVars{baseInd,1} = clusterVars{baseInd,1} .* demPt(dInd).pdf.fft(fftInd,1:lenFFT);
        clusterVars{baseInd,2} = clusterVars{baseInd,2} + (demPt(dInd).xPos - rivPt.xPos)^2 ...
            + (demPt(dInd).yPos - rivPt.yPos)^2;
    %     clusterVars{baseInd,3} = clusterVars{baseInd,3} + [];
    else
        clusterVars{baseInd,1} = demPt(dInd).pdf.fft(fftInd,1:lenFFT);
        clusterVars{baseInd,2} = (demPt(dInd).xPos - rivPt.xPos)^2 ...
            + (demPt(dInd).yPos - rivPt.yPos)^2;
        for i = 1:length(clusterInd)
            clusterVars{baseInd,1} = clusterVars{baseInd,1} .* demPt(clusterInd(i)).pdf.fft(fftInd,1:lenFFT);
...
elseif strcmp(task, 'rem')
    ...
    lenConv = (length(clusterInd))*len - (length(clusterInd)-1);
    lenFFT = max(pow2(nextpow2(lenConv)), length(clusterVars{baseInd,1}));
    fftInd = log2(lenFFT) - 5;
    clusterVars{baseInd,1} = clusterVars{baseInd,1} ./ demPt(dInd).pdf.fft(fftInd,1:lenFFT);
    clusterVars{baseInd,2} = clusterVars{baseInd,2} - (demPt(dInd).xPos - rivPt.xPos)^2 ...
